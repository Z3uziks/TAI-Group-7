    ...
    double estimate_bits(const std::string& sequence) {
        ...
        for (size_t i = 0; i < sequence.length() - k; ++i) {
            ...
            if (counts.find(context) != counts.end()) {
                const auto& context_counts = counts[context];
                ...
                int symbol_count = 0;
                if (context_counts.find(next_symbol) != context_counts.end()) {
                    symbol_count = context_counts.at(next_symbol);
                }
                ...
                // Context not seen in training - use uniform distribution with smoothing
                double prob = alpha / (alpha * alphabet_size);
                double bits = -std::log2(prob);
                total_bits += bits;
            }
        }
